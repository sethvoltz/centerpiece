...
class Encoder(object):
    def __init__(self, pin_clk, pin_dt, pin_mode=None, clicks=1,
                 min_val=0, max_val=100, accel=0, reverse=False, wrap=False):
    ...
    def _callback(self, line):
        ...
        if self._state:
            ...
            calculated = self._value + (1 + (self.cur_accel >> ACCEL_THRESHOLD)) * self._state
            if self.wrap:
                diff = self.max_val - self.min_val
                self._value = (((calculated - self.min_val) % diff) + self.min_val)
            else:
                self._value = min(self.max_val, max(self.min_val, calculated))
